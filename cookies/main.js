...
inputs.forEach( input => {
    input.addEventListener("invalid", handleValidator)
    input.addEventListener("input", handleValidator)
})

function handleValidator(e) {
   if(e.type === "invalid") {
    e.target.setCustomValidity("Ce champs doit etre remplie !");
   }
...
function displayCookies() {
    ...
    if(!cookies[0]) {
        ...
        lock = true;
        infoTxt.textContent = "Pas de cookies a afficher, crée en un !";
